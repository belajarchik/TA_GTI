...
void drawMenu()
{
    ...
    float boxW = 530.0f; // Lebar kotak menu
    float boxH = 280.0f; // Tinggi kotak menu
    ...
    // Petunjuk Kontrol
    const char *baris1 = "Gunakan Arrow Kiri/Kanan untuk berpindah lane";
    const char *baris2 = "Gunakan W/A/S/D untuk menggeser kamera";
    const char *baris3 = "Raih 1000 poin, tanpa menabrak asteroid untuk escape";
    ...
    for (p = baris1; *p; p++)
    {
        glutBitmapCharacter(fontBiasa, *p);
    }
...
void drawWin(int score, float seconds)
{
    ...
    begin2D(); /* pastikan begin2D() & drawText() sudah bisa diakses */
    float cx = WIN_W / 2.0f, cy = WIN_H / 2.0f;

    /* Kotak latar semi-transparan */
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    glColor4f(0.0f, 0.0f, 0.0f, 0.55f);
    glBegin(GL_QUADS);
    glVertex2f(cx - 200, cy - 100);
    glVertex2f(cx + 200, cy - 100);
    glVertex2f(cx + 200, cy + 110);
    glVertex2f(cx - 200, cy + 110);
    glEnd();
    glDisable(GL_BLEND);
    ...
    drawText(cx - 52, cy + 78, "YOU WIN!");

    /* Bintang dekoratif */
    glColor3f(1.0f, 0.75f, 0.10f);
    drawText(cx - 95, cy + 52, "* * * * * * * * * * *");

    /* Skor */
    glColor3f(1.0f, 1.0f, 1.0f);
    sprintf(buf, "Skor akhir  :  %d", score);
    drawText(cx - 80, cy + 22, buf);

    /* Waktu */
    sprintf(buf, "Waktu       :  %02d:%02d.%02d", mins, secs, ms);
    drawText(cx - 80, cy - 2, buf);

    /* Garis pemisah */
    glColor3f(0.4f, 0.4f, 0.4f);
    drawText(cx - 95, cy - 22, "- - - - - - - - - - - - - -");

    /* Tombol play again */
    glColor3f(0.3f, 1.0f, 0.5f);
    drawText(cx - 115, cy - 52, "[ SPACE / ENTER ]  Main lagi");

    glColor3f(0.5f, 0.5f, 0.5f);
    drawText(cx - 62, cy - 76, "ESC untuk keluar");
